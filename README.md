...
```typescript
// src/wristband.ts (continued - add to existing file)
import { AuthStrategy } from '@wristband/nextjs-auth';
...
```typescript
// src/wristband.ts
import { SessionOptions, AuthStrategy } from '@wristband/nextjs-auth';
...
```typescript
// src/wristband.ts
import { createWristbandAuth, AuthStrategy } from '@wristband/nextjs-auth';
...
| --------------------- | ---- | -------- | ------- | ----------- |
| authStrategies | `AuthStrategy[]` | Yes | N/A | Array of authentication strategies to try in sequential order. At least one strategy is required. The middleware will attempt each strategy until one succeeds.<br><br> Available strategies: <br>- `AuthStrategy.SESSION` (cookie-based sessions)<br>- `JWT` (bearer token authentication) |
| sessionConfig | object | Required if using `AuthStrategy.SESSION` strategy | N/A | Configuration object for session-based authentication. Must be provided when `AuthStrategy.SESSION` is included in authStrategies. Contains session options, CSRF settings, and endpoint paths. |
| jwtConfig | object | No | `undefined` | Optional configuration object for JWT bearer token authentication. Contains caching settings for JSON Web Key Sets (JWKS) used to verify JWT signatures. This is only needed if you are using the `AuthStrategy.JWT` strategy and don't want to rely on default config values. |
| protectedApis | string[] | No | `[]` (no API routes protected, **except** the Session and Token Endpoints which are automatically protected when using the `AuthStrategy.SESSION` strategy) | Array of regex patterns defining which API routes require authentication. Routes matching these patterns return HTTP 401 if authentication fails. Patterns support exact paths (`'/api/users'`), wildcards (`'/api/v1(.*)'`), and named parameters (`'/api/users/:id'`). The default is an empty list meaning <br><br> Example: [`'/api/v1(.*)'`, `'/api/v2/orders/:id'`] protects all v1 APIs and specific v2 order routes.<br><br> Important: Session and Token Endpoints are automatically protected when using the `AuthStrategy.SESSION` strategy and don't need to be listed here. |
...
| onPageUnauthenticated | `(req: NextRequest) => NextResponse \| Promise<NextResponse>` | No | A function that redirects to your Login Endpoint. | Custom handler for unauthenticated requests to protected pages. Receives the `NextRequest` and must return a `NextResponse`.<br><br> Example use cases: Custom login pages, error pages, different redirects based on the route. See examples below for custom implementations. |
...
> **JWT and Page Routes**
> The `JWT` strategy is designed with API routes in mind. While it technically works for protected pages, browsers don't naturally send JWT tokens in Authorization headers during page navigation. Use `SESSION` strategy for traditional web page protection.
...
| reason | string or `undefined` | Reason for failure if `authenticated` is `false`.<br><br> Possible values: `not_authenticated`, `token_refresh_failed`, `unexpected_error`. |
| session | `MutableSession` or `null` | The authenticated session (only present if `authenticated` is `true`). |
